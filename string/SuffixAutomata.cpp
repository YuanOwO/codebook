...
struct SAM{
    ...
    void push(const string& s){for (char c : s) add(c - 'a');} 
    void add(int ch) {
        int cur = sz++;
    ...
    void topo() { // counting sort + bottom-up sz update
        int max_len = 0;
        for (int i = 0; i < sz; i++) {
    ...
    void lcs(const string& t){ //substring
        int cur = 0, step = 0;
        for (int i = 0; i < t.size(); i++) {
            ...
            // 這裡可以根據題目需求，例如：ans = max(ans, step);
        }
    }
} sam;